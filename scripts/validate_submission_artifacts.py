...
PAPER = ROOT / "paper"
DOWNLOADS_PDF = Path.home() / "Downloads" / "85.pdf"
DESKTOP_PDF = Path.home() / "Desktop" / "85.pdf"
...
def pdf_text_contains(path, tokens):
    try:
        ...
        text = "\n".join(page.extract_text() or "" for page in PdfReader(str(path)).pages[:6])
    except Exception:
        try:
            from PyPDF2 import PdfReader
...
def main():
    ...
    required_summary = [
        ...
        "Negative cases: 24",
        "best_success_reference=robust_mpc_correction",
        "proposal_success=0.48958",
        "best_success=0.59010",
        "paired_success_lower95=-0.11788",
        "main_gate=False",
    ...
    pdf_text_contains(DOWNLOADS_PDF, ["KILL/ARCHIVE", "minimum-intervention", "robust MPC"])
    print(f"validated Paper 85 artifacts: pages={pages}, sha256={sha256(DOWNLOADS_PDF)}")
